...
<script setup>  
    ...
    const distance = ref(null); // Distance from the current location
    ...
    const fetchEventDetails = async () => {
        try {
            ...
            if (eventSnap.exists()) {
                ...
                initMap(); // Initialize the map after fetching event details
            } else {
    ...
    const initMap = () => {
  if (!event.value || !event.value.location) return; // Ensure event data is available

  const loader = new Loader({
    apiKey: PLACES_API_KEY, // Replace with your API key
    version: 'weekly',
    libraries: ['places'],
  });
